...
# Copyright (c) 2016 Pluribus Networks Inc.
#
...
enable_tpm2 = True
try:
	from OpenSSL.SSL import ContextFromCPtr
except ImportError:
	enable_tpm2 = False
...
try:
        libswitchkey = ctypes.cdll.LoadLibrary("libswitchkey.so")
except:
        libswitchkey = None
        pass
...
class CurlTransportEngine(TransportEngine):
        ...
        def __setup_handle(self, hdl, treq):
                ...
                if treq.sslkey:
                        ...
                        if libswitchkey:
                            if enable_tpm2 and libswitchkey.nvOS_has_tpm_privkey():
                                _nvOS_get_tpm_sigalgs = libswitchkey.nvOS_get_tpm_sigalgs
                                _nvOS_get_tpm_sigalgs.restype = ctypes.c_char_p
                                hdl.setopt(pycurl.SSL_CTX_FUNCTION,
                                    lambda ptr: ContextFromCPtr(ptr).set_client_sigalgs_list(_nvOS_get_tpm_sigalgs()))
				hdl.setopt(pycurl.SSLENGINE, 'tpm2.0')
				hdl.setopt(pycurl.SSLKEYTYPE, 'ENG')
                            else:
                                buf = ctypes.create_string_buffer(100)
                                libswitchkey.nvOS_get_privkey_pass(buf,
                                    ctypes.sizeof(buf))
                                hdl.setopt(pycurl.SSLKEYPASSWD, buf.value)
                                # clear out the buffer
                                buf.raw = '\0' * ctypes.sizeof(buf)
